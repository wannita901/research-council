...
def _events():
    return [
        {"run_id": "r1", "kind": "topic", "payload": {"topic": "LLM code review vs SAST"}},
        {
            ...
            "payload": {
                "codename": "Aiden",
                "gap": "no class-stratified eval",
                "refs": ["openalex:W1", "arxiv:2401.1"],
            },
        },
        {
            "run_id": "r1",
            ...
            "payload": {
                "codename": "Cathy",
                "gap": "triage cost ignored",
                "refs": ["openalex:W1", "missing:X"],
            },
        },
        {
            "run_id": "r1",
            ...
            "payload": {
                "id": "Aiden",
                "title": "Class-stratified eval",
                "hypothesis": "LLM wins on some classes",
...
def _papers():
    return {
        "openalex:W1": Paper(
            id="openalex:W1",
            title="LLMs for vuln detection",
            abstract="abstract one",
            source="openalex",
        ),
        "arxiv:2401.1": Paper(
            id="arxiv:2401.1", title="SAST limits", abstract="abstract two", source="arxiv"
        ),
    ...
    async def route(self, source, *, updated=None):
        ...
        return [
            WikiPage(
                ...
                updated=updated or "2026-06-07",
                body=source.text[:200],
            )
        ]
...
def test_build_internal_synthesizes_findings():
    ...
    assert (
        build_internal([{"run_id": "x", "kind": "topic", "payload": {"topic": "t"}}], "t", "x")
        is None
    )
