...
class TraceWriter:
    ...
    def emit(self, phase: str, kind: str, payload: dict, *, round: int = 0,
             author_vendor: str | None = None) -> Event:
        ev = Event(run_id=self.run_id, ts=_now(), phase=phase, round=round,
                   author_vendor=author_vendor, kind=kind, payload=payload)
