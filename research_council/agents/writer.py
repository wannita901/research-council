...
def _refs_block(citations: list[Citation]) -> str:
    ...
    return "Prior-art references you MAY cite (use the exact key; cite none if irrelevant):\n" + "\n".join(lines)
...
def _experiment_block(experiment: dict) -> str:
    ...
    if rqs:  # RQ-driven results (plan/21): one row per research question
        ...
        for rr in rqs:
            res = rr.get("result", {})
            lines.append(f"- {rr.get('rq_id', '')}: {rr.get('question', '')} → "
        ...
        return "\n".join(lines) + "\n"
    return (f"Experiment result: ran={experiment.get('ran')} feasible={experiment.get('feasible')} "
...
class Writer:
    def __init__(self, model, *, venue: str = "a CS venue", price_model: str | None = None):
        ...
        self._agent: Agent = Agent(model, output_type=PaperDraft,
                                   system_prompt=prompts.load("writing/writer", venue=venue))

    async def draft(self, idea: dict, experiment: dict, constraints: dict | None = None,
                    *, allowed_citations: list[Citation] | None = None, figure: str = "") -> PaperDraft:
    ...
    async def revise(self, draft: PaperDraft, change_requests, sections: list[str]) -> PaperDraft:
        ...
        reqs = "\n".join(f"- [{c.severity}] ({c.section or 'whole'}) {c.msg}" for c in change_requests)
        prompt = (f"Current title: {draft.title}\nCurrent abstract: {draft.abstract}\n\n"
    ...
    async def coherence_pass(self, draft: PaperDraft) -> PaperDraft:
        ...
        prompt = (f"Final coherence pass. Title: {draft.title}\nAbstract: {draft.abstract}\n\n{body[:5000]}\n\n"
...
class PaperReviewer:
    def __init__(self, model, *, venue: str = "a CS venue", vendor: str = "",
                 price_model: str | None = None):
        self.vendor = vendor
        self._price_model = price_model
        self.usage = UsageMeter()
        self._agent: Agent = Agent(model, output_type=ReviewNotes,
...
class VenueRecommender:
    ...
    def __init__(self, model, *, price_model: str | None = None):
        self._price_model = price_model
        self.usage = UsageMeter()
        self._agent: Agent = Agent(model, output_type=VenueChoice,
                                   system_prompt=prompts.load("writing/venue_recommender"))

    async def recommend(self, idea: dict, experiment: dict, venues: list[str]) -> VenueChoice:
        prompt = (f"Available venues: {', '.join(venues)}\n\n"
