...
class StubV2Peer:
    ...
    async def propose(self, brief: ResearchBrief, constraints_text: str = "") -> Candidate:
        return Candidate(
            id=self.codename,
            vendor=self.vendor,
            title=f"[{self.codename}] idea",
            gap=brief.gap,
            problem_statement="stub problem statement",
            motivation="stub motivation",
            hypothesis="stub hypothesis",
            method="stub method",
            experiment_plan="1. toy step\n2. measure",
            research_questions=[
                ResearchQuestion(
                    id="rq1",
                    question="does the toy method work?",
                    plan="1. run toy\n2. measure",
                    metrics="accuracy",
                ),
                ResearchQuestion(
                    id="rq2",
                    question="does it beat the baseline?",
                    plan="1. run baseline\n2. compare",
                    metrics="accuracy",
                ),
            ],
            dataset_metrics="synthetic toy data · accuracy",
            fallback_plan="simplify to a smaller toy run",
            refs=brief.refs,
        )
    ...
    async def deliberate(
        ...
        *,
        ...
        already_revised = any(
            m.from_codename == self.codename and m.kind == "revise" for m in thread
        )
        if candidates[0].id == self.codename and not already_revised:
            return Contribution(
                kind="revise",
                targets=self.codename,
