...
class LLMPeer:
    ...
    async def propose(self, brief: ResearchBrief) -> Candidate:
        ...
        return Candidate(
            ...
            gap=brief.gap,
            hypothesis=_text(d.get("hypothesis")),
            method=_text(d.get("method")),
            experiment_plan=_text(d.get("experiment_plan")),
            refs=brief.refs,
        )

    async def critique(self, anon: list[dict]) -> list[Critique]:
        out: list[Critique] = []
        for i in _json(await self._text(CRITIQUE_SYS, json.dumps(anon), "critique")).get(
            "items", []
        ):
            ...
            out.append(
                Critique(
                    ...
                    target_id=str(i["label"]),
                    axis=str(i.get("axis", "soundness")),
        ...
        for i in _json(await self._text(SCORE_SYS, payload, "score")).get("items", []):
            ...
            out.append(
                Score(
                    ...
                    candidate_id=str(i["label"]),
                    novelty=_num(i.get("novelty")),
                    soundness=_num(i.get("soundness")),
                    feasibility=_num(i.get("feasibility")),
                    clarity=_num(i.get("clarity")),
                )
            )
        return out
