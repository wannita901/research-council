...
def _merge(existing: WikiPage, new: WikiPage) -> WikiPage:
    ...
    return WikiPage(
        type=existing.type, title=existing.title, slug=existing.slug, origin=origin,
        papers=_dedup(existing.papers + new.papers),
        related=_dedup(existing.related + new.related),
        updated=new.updated or existing.updated, body=body,
    )


class Ingestor:
    def __init__(self, librarian, knowledge_root: Path | str | None = None):
        self.librarian = librarian  # any object with async route(Source, updated=) -> list[WikiPage]
