...
def _better(a: ExperimentResult | None, b: ExperimentResult) -> ExperimentResult:
    ...
    if a is None:
        return b
    key = lambda r: (r.approved, r.feasible, r.approvals, r.iterations)
    return b if key(b) > key(a) else a


async def _run_probe(review: CodeReview, sandbox, timeout: int, emit: Emit, attempt: int) -> CodeReview:
    ...
    for f in review.findings:
        if f.probe and (f.probe.code or "").strip():
            ...
            if emit:
                emit("experiment", "review_probe",
                     {"attempt": attempt, "vendor": review.reviewer_vendor,
                      "kind": f.kind, "ran": res.ok, "backend": res.backend})
...
async def run_experimentation(idea: dict, plan: str, coder, reviewers, sandbox, *,
    ...
    for attempt in range(1, caps.max_iters + 1):
        ...
        if emit:
            emit("experiment", "sandbox_run",
                 {"attempt": attempt, "ok": res.ok, "exit_code": res.exit_code,
                  "timed_out": res.timed_out, "feasible": feasible, "backend": res.backend})
        ...
        for rv in reviewers:
            ...
            if emit:
                emit("experiment", "code_review",
                     {"attempt": attempt, "vendor": review.reviewer_vendor,
                      "approve": review.approve, "blocker": review.has_blocker,
        ...
        cur = ExperimentResult(
            ran=last_ran, feasible=feasible, metric=metric, attempts=attempt, iterations=attempt,
            code=code, log=(res.stdout or res.stderr or f"exit {res.exit_code}")[-1200:],
            backend=res.backend, approved=approved, approvals=approvals, reviews=reviews, usd=spent)
        best = _better(best, cur)

        if approved:
            cur.stopped_reason = "approved"
            if emit:
                emit("experiment", "approved", {"attempt": attempt, "approvals": approvals, "usd": spent})
            return cur
    ...
    if emit:
        emit("experiment", "iters_exhausted", {"iterations": caps.max_iters, "approvals": best.approvals})
    return best
    ...
    for rq in rqs:
        ...
        if emit:
            emit("experiment", "rq_done",
                 {"rq_id": rq.id, "feasible": res.feasible, "approved": res.approved,
                  "metric": res.metric})
    return out
...
def write_experiments(rq_results: list[RQResult], out_dir: Path | str) -> Path:
    ...
    for rr in rq_results:
        ...
        (sub / "question.md").write_text(f"# {rr.rq_id.upper()}\n\n{rr.question}\n", encoding="utf-8")
        name, val = _metric_parts(r.metric)
        rows.append({"rq_id": rr.rq_id, "question": rr.question, "metric": name, "value": val,
                     "feasible": r.feasible, "approved": r.approved, "approvals": r.approvals,
                     "iterations": r.iterations, "stopped_reason": r.stopped_reason,
                     "backend": r.backend})
    with (exp / "results.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["rq_id", "question", "metric", "value", "feasible",
                                          "approved", "approvals", "iterations", "stopped_reason",
                                          "backend"])
        w.writeheader()
        w.writerows(rows)
    return exp
...
def _reviews_md(result: ExperimentResult) -> str:
    ...
    for r in result.reviews:
        rv.append(f"## {r.reviewer_vendor or 'reviewer'} — {'approve' if r.approve else 'request changes'}")
        if r.summary:
            rv.append(r.summary)
        for f in r.findings:
            rv.append(f"- **[{f.severity} {f.kind}]** {f.msg}" + (f" → _fix:_ {f.fix}" if f.fix else ""))
            if f.probe and f.probe.ran:
                rv.append(f"  - probe ({'supports' if f.probe.supports else 'inconclusive'}): {f.probe.output[:200]}")
        rv.append("")
    return "\n".join(rv) + "\n"
